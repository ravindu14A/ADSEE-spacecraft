...
def calculate_cg_limits(plot=True):
    ...
    Loading order (per assignment):
        1. OEW (including batteries)  — starting point
        2. Cargo                      — two ordering paths A / B
        3. Window-seat pax            — front-to-back A, back-to-front B
        4. Aisle-seat pax             — front-to-back A, back-to-front B
        5. Fuel                       — up to MTOW
    ...
    # Compute EOW CG automatically from CG_positions_updated
    cg_results = cg.calculate_aircraft_cgs(X_LEMAC)
    X_EOW      = cg_results["from_nose"]["aircraft"]

    # Weights  (all come directly from the corrected Input_updated)
    OEW        = ip.EOW                    # includes batteries
    MTOW       = ip.MTOW
    MZFW       = ip.MZFW_EXX
    MAX_FUEL   = ip.MAX_ALLOWABLE_FUEL_EXX
    X_FUEL     = ip.X_FUEL
    ...
    # ------------------------------------------------------------------
    pa_W  = [OEW];  pa_CG  = [X_EOW]      # Path A (forward-first)
    pb_W  = [OEW];  pb_CG  = [X_EOW]      # Path B (aft-first)
    ...
    n_window = n_cargo + NUM_ROWS            # next NUM_ROWS points
    n_aisle  = n_window + NUM_ROWS           # remaining points
    ...
    if plot:
        ...
        print("=" * 50)
        print(f"  EOW weight      : {OEW:.1f} kg")
        print(f"  EOW CG          : {X_EOW:.3f} m from nose")
    ...
    if plot:
        ...
        # --- Reference markers ---
        ax.scatter([pa_mac[0]], [OEW], color='white', s=120, zorder=5,
                   label=f'OEW ({OEW:.0f} kg)')
